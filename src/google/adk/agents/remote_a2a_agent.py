...
class RemoteA2aAgent(BaseAgent):
  ...
  def _construct_message_parts_from_session(
    ...
    """
    logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Starting for agent: {self.name}")
    logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Session ID: {ctx.session.id}")
    logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Total events in session: {len(ctx.session.events)}")
    
    message_parts: list[A2APart] = []
    context_id = None
    event_count = 0
    
    logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Processing events in reverse order for agent: {self.name}")
    
    for event in reversed(ctx.session.events):
      event_count += 1
      logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Processing event #{event_count} for agent: {self.name}")
      logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Event author: {event.author}")
      logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Event has content: {event.content is not None}")
      logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Event has parts: {event.content.parts if event.content else 'N/A'}")
      logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Event custom_metadata: {event.custom_metadata}")
      
      if _is_other_agent_reply(self.name, event):
        logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Event is other agent reply for agent: {self.name}")
        event = _present_other_agent_message(event)
        logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] After presenting other agent message for agent: {self.name}")
      elif event.author == self.name:
        logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Event is from current agent, stopping processing for agent: {self.name}")
        ...
        if event.custom_metadata:
          ...
          logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Found context_id: {context_id} for agent: {self.name}")
        else:
          logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] No custom_metadata for current agent event for agent: {self.name}")
        break

      if not event.content or not event.content.parts:
        logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Skipping event with no content/parts for agent: {self.name}")
        continue

      logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Processing {len(event.content.parts)} parts for agent: {self.name}")
      
      for part_index, part in enumerate(event.content.parts):
        logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Processing part #{part_index + 1} for agent: {self.name}")
        logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Part type: {type(part)}")
        logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Part content: {part}")

        converted_part = self._genai_part_converter(part)
        if converted_part:
          logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Successfully converted part for agent: {self.name}")
          logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Converted part: {converted_part}")
          message_parts.append(converted_part)
        else:
          logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Failed to convert part to A2A format for agent: {self.name}")
          logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Failed part: {part}")
          logger.warning("Failed to convert part to A2A format: %s", part)
    
    logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Completed processing for agent: {self.name}")
    logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Total events processed: {event_count}")
    logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Total message parts created: {len(message_parts)}")
    logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Context ID: {context_id}")
    logger.error(f"[RemoteA2aAgent._construct_message_parts_from_session] Message parts: {message_parts}")
